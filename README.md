...
| --- | --- |
| pedro.agrelo.marquez@udc.es | Pedro José Agrelo Márquez |
| hugo.garabatos@ud.es | Hugo Garabatos Díaz |
...
* ***(0.25 puntos)*** Documenta el código, explicando qué hace cada una de las funciones. En concreto, se pide tener especial atención al funcionamiento del algoritmo:
  ...
  El estado inicial es una lista vacía, sin reinas colocadas
  2. ¿Dado un estado, cómo se escogen los vecinos?
    Se agrega una nueva reina en la siguiente fila disponible
  3. ¿Cuándo finaliza el algoritmo?
  Cuando se han colocado el número total de reinas y se cumplen las condiciones del problema.
  4. ¿Qué utilidad tienen las variables incluídas en la clase ColaDePrioridad?
      - **`cp`**: Almacena los elementos de la cola de prioridad ordenados por coste.
      - **`buscador`**: Un diccionario que permite acceder rápidamente a las soluciones almacenadas.
      - **`REMOVED`**: Marcador para identificar nodos eliminados y evitar su uso en la búsqueda.
